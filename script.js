...
function loginAdmin() {
  ...
  if (code !== '231005') {
    ...
    if (status) status.textContent = '❌ KODE ADMIN SALAH';
    return;
  }
  ...
  if (window.mbgAuth?.configured && typeof window.mbgAuth.loginAdmin === 'function') {
    window.mbgAuth.loginAdmin(code)
      .then(() => renderAdminUsers())
      .catch(() => {
        const status = document.getElementById('admin-status');
        if (status) status.textContent = 'Mode lokal aktif. Firebase belum siap untuk data user.';
      });
  }
}
...
async function renderAdminUsers() {
  ...
  if (!sessionStorage.getItem('mbgSessionToken')) {
    status.textContent = 'Mode lokal admin aktif. Konfigurasikan Firebase untuk memuat data user.';
    return;
  }
  try {
...
function bindEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (btn.id === 'create-user') {
      ...
      try {
        const result = await createUserAccount(input.value);
        input.value = '';
    ...
    if (btn.id === 'admin-check-inactive') {
      try {
        ...
        await renderAdminUsers();
        toast(`${result.disabled || 0} akun dinonaktifkan.`);
      } catch (error) {
        document.getElementById('admin-status').textContent = error.message;
      }
      return;
